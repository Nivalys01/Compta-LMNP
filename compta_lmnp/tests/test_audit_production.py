...
def test_aucun_placeholder_de_titulaire():
    ...
    for f in (glob.glob(os.path.join(HERE, "**/*.py"), recursive=True)
              ...
              + [os.path.join(os.path.dirname(HERE), "LICENSE")]):
...
def _racine(nom):
    return open(os.path.join(RACINE_DEPOT, nom), encoding="utf-8").read()


def test_licence_est_bien_lagpl_complete():
    """La licence livrée doit être le texte INTÉGRAL de l'AGPL, pas un
    résumé. Un résumé n'est pas opposable, et une licence tronquée ne
    concède rien de ce qu'elle a l'air de concéder."""
    lic = _racine("LICENSE")
    assert "GNU AFFERO GENERAL PUBLIC LICENSE" in lic
    assert "Version 3, 19 November 2007" in lic
    for section in ("0. Definitions.",
                    # la clause propre à l'AGPL : sans elle, c'est une GPL
...
def test_licence_livree_avec_le_paquet_et_source_offerte():
    """L'AGPL impose deux gestes matériels : livrer la licence AVEC le
    programme (art. 4), et offrir la source à qui utilise le logiciel à
    travers un réseau (art. 13). L'application étant un serveur web, le
    second n'est pas théorique."""
    ...
    assert "AGPL" in footer                          # sous quelle licence
    assert "github.com" in footer                    # où prendre la source


def test_notices_tierces_disent_les_vraies_dependances():
    """Le document d'accueil a affirmé pendant des mois que le logiciel
    n'utilisait « que la bibliothèque standard de Python », alors que Flask
    était une dépendance d'exécution obligatoire. Une affirmation de ce
    type, dans la section licence, est une déclaration de conformité
    fausse."""
    notices = _racine("NOTICES-TIERS.md")
    for composant in ("Flask", "reportlab", "BSD-3-Clause", "MIT"):
        assert composant in notices, composant
    accueil = open(os.path.join(HERE, "LISEZ-MOI.md"), encoding="utf-8").read()
    for faux in ("n'utilise **que la bibliothèque standard",
                 "que la bibliothèque standard de Python**"):
        assert faux not in accueil, faux


def test_readme_racine_porte_les_avertissements():
    """La page d'accueil d'un dépôt public est le seul document dont on est
    sûr qu'il sera lu. L'avertissement fiscal et le régime des données
    personnelles n'ont pas à être cherchés trois niveaux plus bas."""
    readme = _racine("README.md")
    for attendu in ("AGPL-3.0-or-later", "aides à la préparation",
                    "RGPD", "responsable de traitement",
                    "aucune garantie"):
        assert attendu in readme, attendu
