...
| Échéances | `echeancier.py` | Moteur GÉNÉRIQUE de génération en lot : aperçu (statuts, doublons probables), génération atomique par le guichet, idempotence par la base (`echeance_generee`, unique par source × date, et par source × rang quand la source numérote ses échéances). Une source lui remet des `Echeance` à une ou plusieurs opérations, en une écriture chacune ou en UNE écriture ventilée. Calendrier ancré commun (`date_ancree`). |
...
| Emprunts | `emprunts.py`, `emprunts_web.py`, `emprunts_cli.py` | Seconde source du moteur. Tableau de remboursement en centimes entiers et `Decimal` (la banque fait foi : calcul proposé, lignes remplaçables, import CSV strict), invariants revérifiés sur le tableau relu en base, verrou des rangs passés en écriture. Une échéance = UNE écriture (`operations.saisir_ventilee` : 661100 + 616110 / 108000, une opération par part) ; le capital n'est pas comptabilisé. Contrôles `EMPRUNT_*` dans `controles.py`. |
